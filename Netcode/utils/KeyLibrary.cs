...
public class KeyLibrary//处理的是未经NetcodeTool格式处理的信息，即"占用"中的信息
{
    ...
    private Action<ENCKey, List<string>> UpdateEvent;
    private Func<ENCKey,bool> RecvEvent;
    public KeyLibrary()
    {
        UpdateEvent = (k,r) => 
        {
                ...
                case ENCKey.KeyState.TobeConfirmed:
                    {
                        ...
                        {
                            k.ToConfirmIntervalLeft.ReachAfter(ENCInstance.KeySendInterval);
    ...
    public List<string> Update()//传出[K112233][F]....
    {
        ...
        foreach (var k in Keys) UpdateEvent.Invoke(k, r);
        if (TimeWiseKeys.Count > 0)
        {
            int index = 0;
            while (index < TimeWiseKeys.Count&&TimeWiseKeys[index].State != ENCKey.KeyState.TobeConfirmed) index++;

            for (int i = Mathf.Min(index,TimeWiseKeys.Count-1); i >= 0; i--) UpdateEvent.Invoke(TimeWiseKeys[i], r);
        }
...
public class ENCKey
{
    ...
    public ENCKey(string data, KeyLibrary.KeyFormatType type)
    {
        ...
        ConfirmedExistingCancel.CancelAfter(ENCInstance.KeyExistTime);
        Key = data;
        Index = UnityEngine.Random.Range(IndexRange, IndexRange * 10 - 1);
        Type = type;
    }
}
public class ENCRKey//仅用于记录回复信息
{
    public string Key;
    public int Index;
    ...
    public ENCRKey(string data, int index)
    {
        ...
        cancel.CancelAfter(ENCInstance.RKeyExistTime);
    }
}